...
class DashboardRepository extends BaseRepository
{
    ...
    public function index($request)
    {
        try {
            ...
            return [
                ...
                'total_refunds' => $this->getTotalRefunds($roleName),
    ...
    public function getTotalRefunds($roleName)
    {
        $refunds = Refund::whereNull('deleted_at');
        if ($roleName == RoleEnum::VENDOR) {
    ...
    public function getOrder($roleName, $request)
    {
        ...
        if ($roleName == RoleEnum::VENDOR) {
            $orders = Order::whereNotNull('parent_id')->where('store_id', Helpers::getCurrentVendorStoreId());
        }
